...
<p align="center">
  ...
  <br>
  <sub>Isaac Sim RTX 렌더. 파이썬이 생성한 USD 를 그대로 올린 것. 25.6 × 17.6 m, 부통로 8개, 진열대 137대, YCB 스캔 상품 27,935개 (실제처럼 빼곡하게)</sub>
</p>
...
### 상품 (YCB 실물 스캔, 실제처럼 빼곡하게)

<p align="center">
  <img src="docs/img/isaac_dense_shelf_face.png" width="720" alt="빼곡한 진열 — 곤돌라 정면">
  <br>
  <sub>곤돌라 정면. 같은 상품 2~5 페이싱이 옆으로 붙고(옆 틈 0.5~1.5 cm), 뒤로는 선반 끝까지, 라벨이 통로를 본다. 처음엔 슬롯마다 6 cm 씩 띄운 "정리된" 진열이었는데 실제와 달라서 바꿨다 — 그러자 평행 그리퍼로 집을 수 있는 상품이 34 % → 0.2 % 로 떨어졌다 (아래 팔 절)</sub>
</p>
...
- **실제처럼 빼곡하게 채운다.** 통로마다 품목군(플래노그램), 같은 상품 2~5 페이싱이 옆으로 붙고(옆 틈 0.5~1.5 cm), 뒤로는 선반 끝까지(최대 6), 라벨(넓은 면)이 통로를 본다. 137대에 27,935개
- **트윈과 시나리오가 같은 코드다.** seed 없이 돌리면 결정적 배치, seed 를 주면 상품 순서·빈 자리가 흔들린다. 그 위에 `scenario.py` 가 넘어짐·오배치·조명을 얹는다 (아래)
- 인스턴싱으로 같은 상품 수천 개가 메시 하나를 공유한다. 27,935개를 올려도 렌더가 몇 분
...
| | seed 7 · ORD_04 |
|---|---|
| **빼곡한 진열**에서 (현재 트윈) | 맨 앞 상품 6,112개 중 그리퍼 폭·높이 310, 옆 틈 확보 343, 셋 다 22, 정차 가능까지 **15개 (0.2 %)**. 그 15개로 40회: 30 성공 (75 %) — 미리 오므린 손가락이 옆면을 밀어 헛잡음 5, 옮기는 중 이탈 4, 옆 줄 쓸어 넘어뜨림 1. **결론: 평행 그리퍼는 실제 진열에 안 맞는다** → 흡착으로 (아래) |
| 이전 슬롯 진열(6 cm 간격)에서, seed 3·7 주문 10건 정차 40곳 | **38/40 성공 (95 %)**. 파지 높이를 중심 − 3 cm 로 바꾸니 약점이던 머스터드 병이 5/7 → 7/7, 토마토 캔 9/9, 스팸 9/9. 남은 실패 2는 옮기는 중 흔들림(세정제 1, 설탕 상자 1)으로 특정 상품·높이와 무관한 잡음. 궤적을 바꾸는 시도 4가지는 40곳 전체에서 오히려 나빠져(29/40) 되돌렸고, 물리 120 Hz 는 같고(38/40), 파지력 100 N 은 나빠서(36/40) 채택하지 않았다 — `docs/LOG.md` (11)(12) |
...
### 흡착 그리퍼 (빼곡한 진열에서 평행 그리퍼가 안 되니까)

<p align="center">
  <img src="docs/img/suction_cup_sweep.png" width="520" alt="컵 지름에 따라 집을 수 있는 상품 비율">
  <br>
  <sub>컵을 키우면 무겁게 들지만(진공력 ∝ 면적) 굴곡을 못 탄다(요철 ∝ 반경²). ⌀20 mm 가 이 상품 구성의 최적이고, ⌀20 + ⌀40 두 컵을 갈아 끼우면 68 %</sub>
</p>

앞면에 진공 컵을 붙여 당겨 뺀다. 옆 틈이 필요 없고 앞에서만 접근하므로 **빼곡한 진열에서 후보가 15개 → 3,008개** 로 늘었다.

| | |
|---|---|
| 컵 사양 | ⌀20 mm 벨로즈, 부압 −60 kPa. 드는 무게 0.48 kg — 수직면이라 무게가 전단이고 진공력 × 마찰이 버틴다: W ≤ μ·(부압 × 면적)/안전율 |
| 상품 판정 | `tools/suction_study.py` — 앞면을 훑어 컵을 앉힐 가장 좋은 자리를 찾고 요철·기울기·면 크기·무게를 본다. 32종 중 61 % 가능. 못 하는 것: 세정제·윈덱스(무거움), 포크·마커·주사위(면이 컵보다 작음), 푸딩 상자·루빅스(기울기) |
| 물리 | Isaac `IsaacSurfaceGripper` + D6 어태치먼트 조인트. 컵이 표면에 2 cm 안으로 닿은 채 켜면 붙고, 축 방향 60 N·전단 25 N 을 넘으면 떨어진다 |
| 파지 배치 | SUCTION_BATCH_README |

**정직한 한계**: 시뮬의 붙음/떨어짐은 힘 한계로만 결정된다. 표면 거칠기·다공성(망사 자루, 골판지 가장자리)·먼지로 진공이 새는 것은 모사하지 않는다. 그래서 `suction_study` 의 기하 필터가 현실이 반영되는 자리이고, 시뮬은 "붙은 뒤 궤적이 버티는가"를 본다.

**잡은 것**: D6 조인트에서 한계를 안 준 축은 자유 축이라 상품이 컵에서 옆으로 미끄러졌다(옆 축 잠금). 컵이 손가락 뒤에 있으면 접근 광선이 자기 손가락에 맞아 안 붙는다(컵을 손끝보다 앞으로). 옮기는 중 손목을 한 번에 90° 꺾으면 전단으로 떨어진다(경유점마다 조금씩 slerp). 곡면 상품은 컵 자리 표면이 AABB 앞면보다 안쪽이라 안 닿는다(그 깊이를 표에 넣는다). `docs/LOG.md` (15)
...
| `tools/verify_stock.py` | 7 | 상품 27,935개 전부: 참조가 풀려 메시가 있는지, 강체·콜라이더·질량, 밑면이 선반에 닿는지(±2 mm), 자기 단 안(지주 사이·앞단 뒤·백판 앞·위 선반 아래)인지, 같은 진열대 안 겹침, 메타데이터 중복 |
...
| `tools/suction_isaac.py` | ✅ | 흡착 그리퍼 (SurfaceGripper). `drive_isaac --arm --suction` |
| `tools/suction_study.py` | ✅ | 상품 앞면 평탄도 → 흡착 가능 판정, 컵 지름 스윕 |
...
tools/
  ...
  suction_isaac.py   흡착 그리퍼 모듈
  suction_study.py   흡착 가능 판정 · 컵 지름 스윕
